...
const validators = {
  ...
  login: (val) => {
    if (!val) return "Le login est obligatoire.";
    if (val.length < 3) return "Le login doit contenir au moins 3 caractères.";
    if (!/^[a-zA-Z0-9_]+$/.test(val)) return "Le login ne doit contenir que des lettres, chiffres et _";
    return "";
  },
...
document.addEventListener("DOMContentLoaded", function () {
  ...
  if (registerForm) {
    ...
    const getValidateLogin = () => validators.login(loginEl.value.trim());
    ...
    attachListeners(loginEl, getValidateLogin);
    ...
    registerForm.addEventListener("submit", function (e) {
      ...
      const validations = [
        ...
        { el: loginEl, fn: getValidateLogin },
